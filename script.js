...
window.navigate = (view, el) => {
    // 1. Temporarily kill CSS smooth scrolling
    document.documentElement.style.scrollBehavior = 'auto';

    // 2. Deactivate all old tabs
    ...
    // 4. 🛡️ THE NUKE: Reset all possible scroll containers
    const snapToTop = () => {
        window.scrollTo(0, 0);
        document.body.scrollTop = 0;
        document.documentElement.scrollTop = 0;
        // In case the main app container is trapping the scroll
        const container = document.querySelector('.app-container');
        if (container) container.scrollTop = 0;
    };
    
    snapToTop(); // Fire instantly
    
    setTimeout(() => {
        snapToTop(); // Fire again exactly after the DOM paints
        // 5. Restore smooth scrolling
        document.documentElement.style.scrollBehavior = 'smooth';
    }, 50);
    
    // 6. Sync Sinking Fund if entering Study tab
...
onAuthStateChanged(auth, async (user) => {
    ...
    // 🛡️ ELITE FIX: Force kill the loader after state is determined
    setTimeout(() => {
        const loader = document.getElementById('initial-loader');
        if(loader) loader.style.display = 'none';
    }, 300); // 300ms buffer for smooth transition
});
...
// ⚡ Elite IndexedDB Wrapper (With Safari Incognito Crash Protection)
const idb = {
    open: () => new Promise((res, rej) => {
        try {
            ...
            req.onerror = () => rej("IDB Blocked by Browser Privacy Mode");
        } catch (e) { rej(e); }
    }),
    get: async (key) => {
        try {
    ...
    set: async (key, val) => {
        ...
        } catch (e) { console.warn("Storage limited. Running in volatile RAM mode."); }
    }
};
...
window.initSQLite = async () => {
    ...
    if (savedDB) {
        ...
        // Ensure new table exists for legacy users updating to v2.0
        localDB.run("CREATE TABLE IF NOT EXISTS deleted_transactions (id TEXT PRIMARY KEY, text TEXT, category TEXT, amount REAL, time INTEGER);");
    } else {
        ...
        localDB.run("CREATE TABLE deleted_transactions (id TEXT PRIMARY KEY, text TEXT, category TEXT, amount REAL, time INTEGER);");
        saveLocalDB();
    }
...
// ⚡ ELITE FIX: Debounced Database Write (Prevents Main Thread Freezing)
...
const saveLocalDB = () => {
    ...
    dbSaveTimeout = setTimeout(() => {
        ...
        console.log("💎 LocalDB Background Sync Complete");
    }, 1000); // Serializes 1 second AFTER the user finishes their action
};
...
// ==========================================
// 💰 ADD TRANSACTION OVERLAY & LOGIC
// ==========================================

window.openTransactionOverlay = () => {
    document.getElementById('transaction-overlay').classList.add('active');
};

window.closeTransactionOverlay = () => {
    document.getElementById('transaction-overlay').classList.remove('active');
};

// Wired to the newly created form
document.getElementById('new-transaction-form').onsubmit = (e) => {
    e.preventDefault();
    if (!localDB) {
        alert("Database initializing... please wait."); // Will replace with Toast later
        return;
    }
    
    // Read from new IDs
    const amt = parseFloat(document.getElementById('new-trans-amount').value);
    const cat = document.getElementById('new-trans-category').value;
    const textRaw = document.getElementById('new-trans-text').value.trim();
    const textValue = textRaw !== "" ? textRaw : "No Description";
    
    // Core SQLite Logic intact
    ...
    saveLocalDB();
    syncLedger(); // This instantly updates the balance, charts, and the History list
    
    e.target.reset(); // Clear inputs
    closeTransactionOverlay(); // Slide screen back down
};

// ==========================================
// 🗑️ SECURE LEDGER RECYCLE BIN LOGIC
// ==========================================

window.deleteTransaction = (docId) => {
    if(confirm("Move this transaction to the Recycle Bin? Ratios and wealth forecast will immediately recalculate.")) {
        // 1. Extract the specific transaction data
        const res = localDB.exec("SELECT id, text, category, amount, time FROM transactions WHERE id = ?", [docId]);
        
        if (res.length > 0) {
            const tx = res[0].values[0];
            // 2. Soft Delete: Insert into deleted_transactions table
            localDB.run("INSERT INTO deleted_transactions (id, text, category, amount, time) VALUES (?, ?, ?, ?, ?)", tx);
            
            // 3. Remove from active transactions
            localDB.run("DELETE FROM transactions WHERE id = ?", [docId]);
            
            saveLocalDB();
            syncLedger(); // Instantly updates home screen
        }
    }
};

window.openDeletedLedger = () => {
    syncDeletedLedger();
    document.getElementById('deleted-ledger-overlay').classList.add('active');
};

window.closeDeletedLedger = () => {
    document.getElementById('deleted-ledger-overlay').classList.remove('active');
};

window.syncDeletedLedger = () => {
    if (!localDB) return;
    
    const res = localDB.exec("SELECT id, text, category, amount, time FROM deleted_transactions ORDER BY time DESC");
    const dt = res.length > 0 ? res[0].values.map(row => ({
        id: row[0], text: row[1], category: row[2], amount: row[3], time: row[4]
    })) : [];

    const historyContainer = document.getElementById('deleted-history-list');
    if (!historyContainer) return;

    if (dt.length === 0) {
        historyContainer.innerHTML = '<p style="text-align:center; color:#94a3b8; font-size:0.8rem; margin-top:20px;">Bin is empty.</p>';
    } else {
        historyContainer.innerHTML = dt.map(tx => {
            const isIncome = tx.amount > 0;
            const amountClass = isIncome ? 'income' : 'expense';
            const prefix = isIncome ? '+' : '';
            const dateStr = new Date(tx.time).toLocaleDateString('en-IN', {day: 'numeric', month: 'short'});
            ...
            return `
                <div class="tx-item" style="opacity: 0.6; border: 1px dashed var(--border);">
                    <div class="tx-info">
                        <span style="text-decoration: line-through;">${tx.text}</span>
                        <div class="tx-meta">${tx.category} • ${dateStr}</div>
                    </div>
                    <div class="tx-actions">
                        <span class="tx-amount ${amountClass}">${prefix}₹${Math.abs(tx.amount).toLocaleString('en-IN')}</span>
                        <div style="display: flex; gap: 5px;">
                            <button onclick="restoreTransaction('${tx.id}')" style="background: rgba(16, 185, 129, 0.1); border: none; color: var(--success); cursor: pointer; padding: 6px; border-radius: 8px;" title="Restore">🔄</button>
                            <button onclick="permanentlyDelete('${tx.id}')" style="background: rgba(244, 63, 94, 0.1); border: none; color: var(--danger); cursor: pointer; padding: 6px; border-radius: 8px;" title="Purge">🔥</button>
                        </div>
                    </div>
                </div>
            `;
...
window.restoreTransaction = (docId) => {
    const res = localDB.exec("SELECT id, text, category, amount, time FROM deleted_transactions WHERE id = ?", [docId]);
    if (res.length > 0) {
        const tx = res[0].values[0];
        localDB.run("INSERT INTO transactions (id, text, category, amount, time) VALUES (?, ?, ?, ?, ?)", tx);
        localDB.run("DELETE FROM deleted_transactions WHERE id = ?", [docId]);
        saveLocalDB();
        syncLedger(); // Update background data
        syncDeletedLedger(); // Update UI list
    }
};

window.permanentlyDelete = (docId) => {
    if(confirm("⚠️ Permanently destroy this record? This action cannot be undone.")) {
        localDB.run("DELETE FROM deleted_transactions WHERE id = ?", [docId]);
        saveLocalDB();
        syncDeletedLedger();
    }
...
function calculateTax(salary) {
    ...
    // Simplified New Regime Tax Slabs
    if (salary > 1500000) liability = (salary - 1500000) * 0.3 + 150000;
    else if (salary > 1200000) liability = (salary - 1200000) * 0.2 + 90000;
    else if (salary > 900000) liability = (salary - 900000) * 0.15 + 45000;
    else if (salary > 600000) liability = (salary - 600000) * 0.1 + 15000;
    else if (salary > 300000) liability = (salary - 300000) * 0.05;

    // 💎 STARTUP FEATURE: Smart Refund Projection
    // Assumes an average 10% TDS has been deducted by employer/clients
    let assumedTDS = salary * 0.10; 
    
    if (salary <= 700000) {
        liability = 0; // Tax Rebate Limit
        refund = assumedTDS; // Full refund potential
    } else {
        // App assumes they will use standard deductions (₹50k) to project a potential refund
        let smartLiability = Math.max(0, liability - 15000); 
        if (assumedTDS > smartLiability) {
            refund = assumedTDS - smartLiability;
        }
    }

    const liabilityEl = document.getElementById('tax-amount');
    const takeHomeEl = document.getElementById('take-home'); // Reused for 'Expected Refund' in HTML

    if (liabilityEl) liabilityEl.innerText = `₹${Math.floor(liability).toLocaleString('en-IN')}`;
    
    if (takeHomeEl) {
        if (refund > 0) {
            takeHomeEl.innerText = `₹${Math.floor(refund).toLocaleString('en-IN')}`;
            takeHomeEl.style.color = "#10B981"; // Wealth Green
        } else {
            takeHomeEl.innerText = "₹0";
            takeHomeEl.style.color = "#94A3B8"; // Muted grey if no refund
        }
    }
}
...
// --- 📚 ELITE GLOBAL STUDY TIMER (Pomodoro) ---
let timeLeft = 1500; // 25 Minutes
timerInterval = null;

// Get both buttons and both displays
const startBtns = [document.getElementById('start-timer'), document.getElementById('home-start-timer')];
const displays = [document.getElementById('timer-display'), document.getElementById('home-timer-display')];

function updateDisplays(mins, secs) {
    const timeString = `${mins}:${secs < 10 ? '0' : ''}${secs}`;
    displays.forEach(d => { if (d) d.innerText = timeString; });
}
...
function toggleTimer() {
    if (timerInterval) {
        ...
        timerInterval = null;
        startBtns.forEach(b => { if (b) b.innerText = "Resume Focus"; });
    } else {
        startBtns.forEach(b => { if (b) b.innerText = "Pause Session"; });
        timerInterval = setInterval(() => {
            timeLeft--;
            const mins = Math.floor(timeLeft / 60);
            const secs = timeLeft % 60;
            updateDisplays(mins, secs);
...
startBtns.forEach(btn => {
    if (btn) btn.onclick = toggleTimer;
});
...
function completeStudySession() {
    ...
    updateDisplays(25, 0);
    startBtns.forEach(b => { if (b) b.innerText = "Start Focus Work"; });
}
...
function updateIdentityUI(data) {
    ...
    // 🛡️ ELITE FIX: Load saved gender into the dropdown
    document.getElementById('set-gender').value = data.gender || '';
...
// ==========================================
// 📊 ELITE APEX CHARTS ENGINE
// ==========================================

function initCharts() {
    // 1. Circular Radial Bar for ACCA Sinking Fund
    const radialOptions = {
        series: [68], // Initial progress percentage
        chart: { type: 'radialBar', height: 120, width: 120, sparkline: { enabled: true } },
        plotOptions: {
            radialBar: {
                hollow: { size: '70%' }, /* 💎 FIX: Made the ring thinner */
                track: { background: '#151A23', strokeWidth: '100%' }, /* 💎 FIX: Exact background dark color */
                dataLabels: {
                    name: { show: false },
                    value: { offsetY: 5, color: '#FFFFFF', fontSize: '1.2rem', fontWeight: 800, show: true }
                }
            }
        },
        stroke: { lineCap: 'round' },
        colors: ['#A78BFA'] // Exact ACCA Purple
    };

    window.sinkingRadialChart = new ApexCharts(document.querySelector("#sinking-radial-chart"), radialOptions);
    window.sinkingRadialChart.render();
}

// Ensure charts load when the DOM is fully ready
document.addEventListener('DOMContentLoaded', () => {
    // Small delay to ensure the div exists
    setTimeout(initCharts, 500); 
});

// 2. Update the Sinking Fund Sync Function to update the chart dynamically
const originalSyncLedger = window.syncLedger;
window.syncLedger = () => {
    originalSyncLedger(); // Run your old logic
    
    // Calculate custom progress for the circular chart
    if(window.sinkingRadialChart) {
        let pct = Math.min((balance / userGoal.target) * 100, 100);
        window.sinkingRadialChart.updateSeries([Math.round(pct)]);
    }
};

// ==========================================
// 📈 TAX INSIGHTS & WEALTH POTENTIAL AUTO-SYNC
// ==========================================

function updateTaxWealthPotential() {
    let progress = typeof currentProgress !== 'undefined' ? currentProgress : 0;
    
    const taxSalary = document.getElementById('tax-career-salary');
    const taxPapers = document.getElementById('tax-career-papers');
    
    if(!taxSalary || !taxPapers) return;

    taxPapers.innerText = `${progress} Papers Unlocked`;

    // Same logic as your ACCA career tab
    if (progress === 0) {
        taxSalary.innerText = "₹2.5L - ₹4L";
    } else if (progress <= 3) {
        taxSalary.innerText = "₹4L - ₹6L";
    } else if (progress <= 9) {
        taxSalary.innerText = "₹7L - ₹12L";
    } else if (progress <= 12) {
        taxSalary.innerText = "₹12L - ₹18L";
    } else {
        taxSalary.innerText = "₹20L - ₹45L+";
    }
}

// Hook this into the existing ACCA update function
const originalUpdateACCAUI = window.updateACCAUI;
window.updateACCAUI = () => {
    if(originalUpdateACCAUI) originalUpdateACCAUI(); // Run old logic
    updateTaxWealthPotential(); // Run new logic for Tax tab
};

// Run once on load to populate data
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(updateTaxWealthPotential, 1000);
});