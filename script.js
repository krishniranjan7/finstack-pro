...
window.verifyPin = () => {
    ...
    if (val.toString() === globalPIN.toString()) {
        ...
        // --- DESKTOP FIX: Only Fullscreen if Mobile ---
...
document.getElementById('transaction-form').onsubmit = async (e) => {
    ...
    // --- EXECUTE FIX: Prevent Null Firestore Pushes ---
    if (!amt || amt <= 0 || !cat) {
        alert("System Warning: Invalid transaction parameters.");
        return;
    }
    ...
    try {
        await addDoc(collection(db, "ledger"), {
            uid: currentUser.uid,
            text: document.getElementById('text') ? document.getElementById('text').value : '',
            category: cat,
            amount: cat === "Income" ? amt : -amt,
            time: serverTimestamp()
        });
        e.target.reset();
        // Sync is handled automatically by your existing onSnapshot listener
    } catch (error) {
        console.error("Ledger Execution Failed:", error);
...
function updateIdentityUI(data) {
    ...
    // --- GENDER FIX: Bind Firestore data to UI ---
    if (data.gender) document.getElementById('set-gender').value = data.gender;
