...
// ==========================================
// 🗄️ SQLITE OFFLINE STORAGE ENGINE
// ==========================================
let localDB = null;
...
window.initSQLite = async () => {
    ...
    const savedDB = localStorage.getItem(`fs_ledger_${currentUser.uid}`);
    
    if (savedDB) {
        localDB = new SQL.Database(fromBase64(savedDB));
    } else {
...
const saveLocalDB = () => {
    localStorage.setItem(`fs_ledger_${currentUser.uid}`, toBase64(localDB.export()));
};
...
// --- 2. SECURE NOTES ENGINE (Bug Fixed) ---
const notesInput = document.getElementById('local-notes');
const notesStatus = document.getElementById('notes-status');

// Isolate notes to the specific authenticated user
window.loadSecureNotes = () => {
    if (!notesInput || !currentUser) return;
    notesInput.value = localStorage.getItem(`fs_notes_${currentUser.uid}`) || '';
    notesStatus.innerText = "Status: Secure";
...
window.saveNotes = () => {
    if (!notesInput || !currentUser) return;
    localStorage.setItem(`fs_notes_${currentUser.uid}`, notesInput.value);
