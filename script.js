...
async function syncSinkingFund() {
    // If Free user, do not waste API calls (Freemium Paywall)
    ...
    const months = parseInt(monthsInput.value) || 12; // Default to 1 year if empty
    const pending = 13 - (currentProgress || 0);

    // If they have cleared everything, show zero
    if (pending <= 0) {
        document.getElementById('inr-sum').innerText = "₹0";
        document.getElementById('monthly-saving-goal').innerText = "₹0 / mo";
        document.getElementById('fund-count-tag').innerText = "All papers cleared! 🎉";
        return;
    }
    ...
    try {
        ...
        document.getElementById('inr-sum').innerText = `₹${Math.floor(totalINR).toLocaleString('en-IN')}`;
        // Math.ceil ensures they save slightly more rather than slightly less
        document.getElementById('monthly-saving-goal').innerText = `₹${Math.ceil(monthlySavings).toLocaleString('en-IN')} / mo`;
        document.getElementById('fund-count-tag').innerText = `${pending} Papers @ ₹${inrRate.toFixed(2)} rate over ${months} months`;
    } catch (e) {
        // Offline Fallback Calculation
        const totalINR = fee * 110 * pending; // Fallback conversion rate
        const monthlySavings = totalINR / months;
        
        document.getElementById('inr-sum').innerText = `₹${Math.floor(totalINR).toLocaleString('en-IN')}`;
        document.getElementById('monthly-saving-goal').innerText = `₹${Math.ceil(monthlySavings).toLocaleString('en-IN')} / mo`;
        document.getElementById('fund-count-tag').innerText = `Offline Mode (Approximate Rate)`;
    }
}

// --- 🗑️ DELETE TRANSACTION ENGINE ---
window.deleteTransaction = async (docId) => {
    // We add a confirmation so users don't accidentally delete critical data
    if(confirm("Delete this transaction? Your wealth forecast will be permanently recalculated.")) {
        try {
            ...
            // Notice: We don't need to manually update the HTML. 
            // The onSnapshot listener in syncLedger will instantly detect the deletion 
            // and refresh the balances and the list automatically.
        } catch (error) {
...
document.getElementById('transaction-form').onsubmit = async (e) => {
    ...
    // Debugging: This will tell you exactly which one is null in the Console
    ...
    if (!amtEl || !catEl || !textEl) {
        console.error("Null check failed:", { amtEl, catEl, textEl });
        alert("SYSTEM ERROR: Inputs not found! Check Console (F12).");
        return;
    }
    ...
    try {
        ...
        e.target.reset();
        alert("💎 Transaction Executed and Synced successfully."); 
    } catch (error) {
        alert("SYSTEM ERROR: " + error.message);
    }
};
