        ...
        if (historyContainer) {
            if (t.length === 0) {
                ...
                historyContainer.innerHTML = t.map(tx => {
                    ...
                    // 🔥 ELITE FIX: Handles both immediate local time and server time without crashing
...
document.getElementById('transaction-form').onsubmit = async (e) => {
    ...
    try {
        ...
        await addDoc(collection(db, "ledger"), {
            ...
            time: new Date() // 🔥 ELITE FIX: Bypasses server delay for instant real-time UI rendering
        });
